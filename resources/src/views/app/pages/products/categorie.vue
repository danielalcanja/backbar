...
export default {
  ...
  methods: {
    ...
    //----------------------------------- Show import Excel -------------------------------\\
    Show_import_category() {
      this.$bvModal.show("importCategory");
    },
    //------------------------------ Event Import  -------------------------------\\
    onFileSelected(e) {
     ...
     Submit_import() {
      ...
      axios
        .post("categories/import/csv", self.data)
        .then(response => {
  ...
  created: function() {
    ...
    Fire.$on("Event_import", () => {
      setTimeout(() => {
        ...
        this.$bvModal.hide("importCategory");
      }, 500);
    });
  }
};
